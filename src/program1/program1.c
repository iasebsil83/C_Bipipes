...
void ____read(bipipe* bp){
	...
	//show data
	if(data == NULL){
		printf("Prog_1 > PID[%i], PPID[%i] : Reading in bipipe [%s] : NULL.\n", getpid(), getppid(), bp->info);
	}else{
		printf("Prog_1 > PID[%i], PPID[%i] : Reading in bipipe [%s] : \"", getpid(), getppid(), bp->info);
...
void ____write(bipipe* bp, char* data){
	...
	printf("Prog_1 > PID[%i], PPID[%i] : Writing in bipipe [%s] : \"%s\".\n", getpid(), getppid(), bp->info, data);
}
...
int main(int argc, char** argv){
	...
	//test procedure (P & P2 runs in parallel)
	____sleep(2.5);

	____read(bp);   //must be "_ Test for P1 _"
	____sleep(2.5);
	...
	//free data
	printf("\nProg_1 > PID[%i], PPID[%i] : Deleting bipipe...\n", getpid(), getppid());
	bipipe_delete(bp);
	printf("Prog_1 > PID[%i], PPID[%i] : Bipipe deleted.\n", getpid(), getppid());
